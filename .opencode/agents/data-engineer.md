---
description: Use when orchestrating a full ETL pipeline workflow, collecting missing details, coordinating specialist agents, and handling issue, branch, commit, and PR steps directly.
mode: primary
---
...
- **dag-airflow**: Use when generating the Airflow DAG file for a pipeline with bootstrap and update flows.
- **sqlalchemy-models**: Use when generating `schemas.py` with SQLAlchemy 2.x models that match the Flyway migrations.
- **eda-reporte**: Use when serializing EDA results into the standardized JSON report consumed by DEA and DB.
- **esquema-db**: Use when deriving the SQL schema and Flyway migrations from the standardized EDA report.
- **git-pipeline-commits**: Use when preparing atomic commits for a pipeline implementation with the repository commit convention.
- **docs-pipeline**: Use when generating the internal pipeline README from implemented artifacts.
- **issue-template**: Use when creating GitHub issues for new pipelines.
- **pull-request-template**: Use when DEA needs to create the pull request for a pipeline.
