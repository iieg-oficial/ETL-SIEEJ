...
LEFT JOIN zonas_geograficas zg ON c.zonas_geograficas_id = zg.id
LEFT JOIN cvegeo_municipalities m ON c.municipios_id = m.id
LEFT JOIN colonias col ON c.colonias_id = col.id
LEFT JOIN calles cal ON c.calles_id = cal.id
LEFT JOIN cruces cr ON c.cruces_id = cr.id;
