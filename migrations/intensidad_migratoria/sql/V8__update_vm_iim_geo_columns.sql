...
SELECT
    m.id AS fid,
    make_date(m.fecha, 1, 1) AS fecha_periodo_seleccion,
    LPAD(s.cve_ent::text, 2, '0')::varchar(2) AS cve_ent,
    s.nom_ent AS entidad,
    LPAD(m.municipio_id::text, 5, '0')::varchar(5) AS cvegeo_municipio,
    ...
    CASE LOWER(BTRIM(
        ...
        WHEN 'muy alto' THEN 'Muy alto'
        WHEN 'muy bajo' THEN 'Muy bajo'
        ELSE CASE m.fecha
            WHEN 2010 THEN m.gaim
            WHEN 2020 THEN m.gim_dp2
        END
...
COMMENT ON COLUMN vm_iim_geo.fecha_periodo_seleccion IS
    'Fecha de referencia anual del indice.';
...
COMMENT ON COLUMN vm_iim_geo.cvegeo_municipio IS 'Clave INEGI del municipio.';
