...
JOIN cvegeo_states c ON c.cve_ent = e.entidad_id;
...
COMMENT ON VIEW vw_indice_shf_vivienda_estatal IS
    ...
    'Año base 2017 = 100.';
COMMENT ON COLUMN vw_indice_shf_vivienda_estatal.entidad_id IS
...
JOIN cvegeo_municipalities c ON c.cvegeo = m.municipio_id;
...
COMMENT ON VIEW vw_indice_shf_vivienda_municipal IS
    ...
    'resueltos. Año base 2017 = 100.';
COMMENT ON COLUMN vw_indice_shf_vivienda_municipal.municipio_id IS
...
COMMENT ON COLUMN vw_indice_shf_vivienda_municipal.entidad_id IS
...
WHERE m.entidad_id = 14
UNION ALL
SELECT
    'estatal' AS nivel,
    e.nom_ent AS serie,
    NULL::INTEGER AS municipio_id,
    e.fecha,
    e.anio,
    e.trimestre,
    e.indice
FROM vw_indice_shf_vivienda_estatal e
WHERE e.entidad_id = 14
UNION ALL
SELECT
    'global' AS nivel,
    g.serie,
...
COMMENT ON COLUMN vw_indice_shf_vivienda_jalisco.municipio_id IS
