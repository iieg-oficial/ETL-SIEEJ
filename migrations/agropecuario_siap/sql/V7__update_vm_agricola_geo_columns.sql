DROP MATERIALIZED VIEW IF EXISTS vm_agricola_geo;

CREATE MATERIALIZED VIEW vm_agricola_geo AS
SELECT
    sa.id AS fid,
    make_date(sa.anio, 1, 1) AS fecha_periodo_seleccion,
...
COMMENT ON COLUMN vm_agricola_geo.fecha_periodo_seleccion IS
    'Fecha de referencia anual de la produccion agricola.';
...
COMMENT ON COLUMN vm_agricola_geo.cvegeo_municipio IS 'Clave INEGI del municipio.';
