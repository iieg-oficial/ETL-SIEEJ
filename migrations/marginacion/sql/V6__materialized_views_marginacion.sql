...
SELECT
    mm.id AS fid,
    mm.fecha_actualizacion AS fecha,
    LPAD(s.cve_ent::text, 2, '0')::varchar(2) AS cve_ent,
    s.nom_ent AS entidad,
    LPAD(mm.municipio_id::text, 5, '0')::varchar(5) AS cvegeo,
...
COMMENT ON COLUMN vm_marginacion_geo.fecha IS 'Fecha de actualizacion del indice.';
...
COMMENT ON COLUMN vm_marginacion_geo.cvegeo IS 'Clave INEGI del municipio.';
