...
from core.pipelines.enoe.constants import FLOAT_COLS, INT_COLS, NULL_VALUES, RENAME_HEADER
from core.pipelines.enoe.mappings import (
    EstadoCivil,
    NivelEducativo,
    Ocupacion,
    Sector,
...
_PERIOD_RE = re.compile(r"enoe_(\d{4})_(\d)\.pkl$")


class EnoeTransform(Stage):
    ...
    def source(self, input_data: Optional[Any] = None) -> list[Path]:
        ...
        pkls = sorted(extract_dir.glob("enoe_*.pkl"))
        if pkls:
    ...
    def _build_catalogs(self) -> dict:
        return {
            T.CAT_SECTOR: Sector.to_records("descripcion"),
            T.CAT_OCUPACION: Ocupacion.to_records("descripcion"),
            T.CAT_SITUACION_TRABAJO: SituacionTrabajo.to_records("descripcion"),
            T.CAT_TIPO_LOCALIDAD: TipoLocalidad.to_records("descripcion"),
            T.CAT_ESTADO_CIVIL: EstadoCivil.to_records("descripcion"),
            T.CAT_NIVEL_EDUCATIVO: NivelEducativo.to_records("descripcion"),
        }
    ...
    def _transform(self, df: pd.DataFrame, anio: int, trimestre: int) -> pd.DataFrame:
        ...
        # cs_p17: 1=habla lengua indígena, 2=no habla, 0=no aplica
        if "cs_p17" in df.columns:
            df["habla_lengua_indigena"] = df["cs_p17"] == 1
            df = df.drop(columns=["cs_p17"])

        # Indicadores derivados — INEGI TIL1
        df["es_pea"] = df["clase1"] == 1
        df["es_ocupado"] = df["clase2"] == 1
        df["es_desocupado"] = df["clase2"] == 2
        df["es_informal"] = (df["clase2"] == 1) & (df["situacion_trabajo_id"] == 1)
    ...
    def action(self, input_data: list[Path]) -> list[dict]:
        ...
        for pkl_path in input_data:
            ...
            df_pkl = self.work_dir / f"enoe_{anio}_{trimestre}.pkl"
    ...
    def finalization(self, input_data: list[dict]) -> list[dict]:
        self.logger.info(f"[finalization] {len(input_data)} trimestres transformed")
        return input_data
