...
class Vialidades(SaludBase):
    ...
    __table_args__ = (UniqueConstraint("vialidad", "tipo_vialidad_id", name="uq_vialidades_nombre_tipo"),)
...
class MarcasMoviles(SaludBase):
    ...
    __table_args__ = (UniqueConstraint("marca", "marca_especifica", "modelo", name="uq_marcas_moviles"),)
...
class Establecimientos(SaludBase):
    ...
    tipo_establecimiento_id: Mapped[int | None] = mapped_column(
        ForeignKey(f"{T.TIPOS_ESTABLECIMIENTO}.id"), nullable=True
    )
