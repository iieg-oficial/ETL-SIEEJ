...
from core.pipelines.emim.schemas import CatActividad, CatEstatus, StgEmim
...
class EmimLoad(Stage):
    ...
    def _load_catalogs(self, session, catalogs: dict[str, list[dict]]) -> dict[str, int]:
        ...
        referenced by stg_emim. cat_actividad is upserted instead of skipped
        because INEGI does reword the SCIAN descriptions between editions, while
        codigo_actividad — the key stg_emim points at — stays put.
        """
    ...
    def action(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            with self.db.get_session() as session:
                ...
                records_before = count_records(session, StgEmim)

                stg_cols = [c for c in StgEmim.columns() if c != StgEmim.id.key]
    ...
    def finalization(self, input_data: dict[str, Any]) -> None:
        ...
        try:
            with self.db.get_session() as session:
                total = count_records(session, StgEmim)
                inserted = total - input_data["records_before"]
            self.logger.info(f"{total:,} records in stg_emim ({inserted:,} new in this run)")
        finally:
            self.db.disconnect()
