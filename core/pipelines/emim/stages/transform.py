...
from core.pipelines.emim.constants import FLOAT_COLS, INT_COLS, NULL_VALUES
...
class EmimTransform(Stage):
    ...
    def action(self, input_data: dict[str, pd.DataFrame]) -> dict[str, Any]:
        ...
        df = df.dropna(subset=["fecha", "entidad_id", "codigo_actividad"])
    ...
    def _estatus_records(df: pd.DataFrame) -> list[dict]:
        """Seed the published statuses in a fixed order, then add anything new.

        The seed goes first so the ids stay stable across editions: which
        statuses a given edition publishes varies, and deriving the order from
        the data would renumber the statuses that stg_emim already points at.
        """
    ...
    def _actividad_records(self, actividades: pd.DataFrame) -> list[dict]:
        """Build the activity catalog, keeping the code as text.

        The sector is published as "31-33", so the code cannot be cast to an
        integer the way EMEC and EMS do it; drop_duplicates_col is avoided here
        because it lowercases, and the code is a key, not a label.
        """
        cat = actividades.copy()
        cat["codigo_actividad"] = cat["codigo_actividad"].str.strip()
        cat["descripcion"] = cat["descripcion"].str.strip()

        cat = cat.replace("", None).dropna(subset=["codigo_actividad", "descripcion"])
