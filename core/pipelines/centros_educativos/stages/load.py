...
from core.utils.bulk_ops import (
    insert_records,  count_records, get_mapping, sync_id_sequence, get_all_records,
)
from core.pipelines.centros_educativos.schemas import (
    Turnos, TiposEducativos, NivelesEducativos, ServiciosEducativos,
    TiposControles, TiposSostenimiento, Localidades, Domicilios,
    Colonias, Centros
)
...
from core.pipelines.centros_educativos.mappings.secondary_tables import (
    TurnosMap, TipoEducativoMap,
    NivelEducativoMap, ServicioEducativoMap,
    NombreControlMap, TipoSostenimientoMap,
)
...
class CentrosEducativosLoad(Stage):
    ...
    def _load_catalogs(self, session, catalogs):
        ...
        insert_records(
                session, TurnosMap.to_records("turno"),
                Turnos, conflict_keys=[Turnos.id.key]
            )

        self.logger.info("[_load_catalogs] Loading tipos_educativos static catalog")
        insert_records(
            session, TipoEducativoMap.to_records("tipo_educativo"),
            TiposEducativos, conflict_keys=[TiposEducativos.id.key]
            )

        self.logger.info("[_load_catalogs] Loading niveles_educativos static catalog")
        insert_records(
            session, NivelEducativoMap.to_records("nivel_educativo"),
            NivelesEducativos, conflict_keys=[NivelesEducativos.id.key]
            )

        self.logger.info("[_load_catalogs] Loading servicios_educativos static catalog")
        insert_records(
            session, ServicioEducativoMap.to_records("servicio_educativo"),
            ServiciosEducativos, conflict_keys=[ServiciosEducativos.id.key]
            )

        self.logger.info("[_load_catalogs] Loading tipos_controles static catalog")
        insert_records(
            session, NombreControlMap.to_records("tipo_control"),
            TiposControles, conflict_keys=[TiposControles.id.key]
            )

        self.logger.info("[_load_catalogs] Loading tipos_sostenimiento static catalog")
        insert_records(
            session, TipoSostenimientoMap.to_records("tipo_sostenimiento"),
            TiposSostenimiento, conflict_keys=[TiposSostenimiento.id.key]
            )

        self.logger.info(f"[_load_catalogs] Loading {len(catalogs[T.LOCALIDADES])} localidades")
        insert_records(
            session, catalogs[T.LOCALIDADES],
            Localidades, conflict_keys=[Localidades.cve_geo_id.key]
            )

        self.logger.info(f"[_load_catalogs] Loading {len(catalogs[T.DOMICILIOS])} domicilios")
        insert_records(
            session,catalogs[T.DOMICILIOS], Domicilios,
        ...
        for colonia_record in catalogs[T.COLONIAS]:
            cve_geo_id = int(f"{colonia_record['entidad_id']:02}{colonia_record['municipio_id']:03}{colonia_record['localidad_id']:04}")
            localidad_id = localidades_map_for_colonias.get(cve_geo_id)
            if localidad_id:
                colonias_records.append({
                    Colonias.colonia.key: colonia_record['colonia'],
                    Colonias.localidad_id.key: localidad_id
                })

        self.logger.info(f"[_load_catalogs] Loading {len(colonias_records)} colonias to database")
        insert_records(
            session, colonias_records,
            Colonias, conflict_keys=[Colonias.colonia.key, Colonias.localidad_id.key]
            )
    ...
    def _map_foreign_keys(self, session, df):
        ...
        turnos_map = get_mapping(
            session, Turnos,
            Turnos.turno.key, Turnos.id.key, is_normalize=True
            )

        self.logger.info("[_map_foreign_keys] Getting tipos_educativos mapping")
        tipos_educativos_map = get_mapping(
            session, TiposEducativos,
            TiposEducativos.tipo_educativo.key, TiposEducativos.id.key, is_normalize=True
            )

        self.logger.info("[_map_foreign_keys] Getting niveles_educativos mapping")
        niveles_educativos_map = get_mapping(
            session, NivelesEducativos,
            NivelesEducativos.nivel_educativo.key, NivelesEducativos.id.key, is_normalize=True
            )

        self.logger.info("[_map_foreign_keys] Getting servicios_educativos mapping")
        servicios_educativos_map = get_mapping(
            session, ServiciosEducativos,
            ServiciosEducativos.servicio_educativo.key, ServiciosEducativos.id.key, is_normalize=True
            )

        self.logger.info("[_map_foreign_keys] Getting tipos_controles mapping")
        tipos_controles_map = get_mapping(
            session, TiposControles,
            TiposControles.tipo_control.key, TiposControles.id.key, is_normalize=True
            )

        self.logger.info("[_map_foreign_keys] Getting tipos_sostenimiento mapping")
        tipos_sostenimiento_map = get_mapping(
            session, TiposSostenimiento,
            TiposSostenimiento.tipo_sostenimiento.key, TiposSostenimiento.id.key, is_normalize=True
            )

        self.logger.info("[_map_foreign_keys] Getting domicilios mapping")
        domicilios_map = get_mapping(
            session, Domicilios,
            Domicilios.domicilio.key, Domicilios.id.key
            )

        self.logger.info("[_map_foreign_keys] Getting colonias mapping")
        colonias_map = get_mapping(
            session, Colonias,
            Colonias.colonia.key, Colonias.id.key
            )
        ...
        df[Centros.tipos_educativos_id.key] = normalize_col(df, TiposEducativos.tipo_educativo.key).map(tipos_educativos_map)
        df[Centros.nivel_educativo_id.key] = normalize_col(df, NivelesEducativos.nivel_educativo.key).map(niveles_educativos_map)
        df[Centros.servicio_educativo_id.key] = normalize_col(df, ServiciosEducativos.servicio_educativo.key).map(servicios_educativos_map)
        df[Centros.tipo_control_id.key] = normalize_col(df, TiposControles.tipo_control.key).map(tipos_controles_map)
        df[Centros.tipo_sostenimiento_id.key] = normalize_col(df, TiposSostenimiento.tipo_sostenimiento.key).map(tipos_sostenimiento_map)

        self.logger.info("[_map_foreign_keys] Mapping geographic locations")
        df['cve_geo_id'] = df.apply(lambda row: int(f"{row['entidad_id']:02}{row['municipio_id']:03}{row['localidad_id']:04}"), axis=1)
